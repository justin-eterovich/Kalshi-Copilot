...
bind-mount the source — **mounting the two source trees, never `/app` itself**:

```bash
docker compose run --rm --no-deps \
  -v ./backend/app:/app/app -v ./backend/tests:/app/tests \
  api python -m pytest tests/ -q
```

Nothing else needs mounting: `compose run` inherits the service's own volumes,
and `x-backend-volumes` already binds `config.yaml`, `data/` and `secrets/`.

**The recipe this replaces (`-v ./backend:/app` plus the files above it) was
wrong in both directions, and it is worth knowing why.** Mounting *over* `/app`
hides `config.example.yaml` and `scripts/`, which are baked in at `/app/` and do
not exist under `backend/` — so `test_config.py` and `test_fee_schedule_script.py`
failed at **collection**, before a single test ran. Mounting them back is what
caused the second problem: Docker creates a missing mount point, and `/app` is
already the host's `./backend`, so every run manufactured four root-owned stubs
on the host — zero-byte `backend/config.yaml` and `backend/config.example.yaml`,
empty `backend/data/` and `backend/scripts/`. That is the exact artifact commit
`3a93b4e` exists to prevent, regenerated by this file's own instructions.

The sharp end is not the clutter. `backend/config.yaml` is gitignored so it
never appears in `git status`, but **`backend/config.example.yaml` is not** — it
shows up as untracked, one `git add -A` away from committing an empty template
over the tracked one `test_config.py` asserts the shipped-safe posture against.
The corrected command creates nothing, because `/app/app` and `/app/tests`
already exist in the image; it also removes the need for `--cache-dir /tmp/...`
and `-p no:cacheprovider`, which were the same root cause wearing a different
hat. It still tests the working tree rather than the image, so a change to
`pyproject.toml`, `Dockerfile`, `config.example.yaml`, `scripts/` or `data/`
